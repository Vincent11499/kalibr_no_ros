"""Execution controls and optional profiling for Kalibr calibration.

Parallelism overrides are applied only when explicitly requested. Timing and
memory measurements require a profiling build and an explicit output path.
"""
...
def add_parallelism_arguments(argument_container):
    ...
    if _profiling_enabled:
        ...
        argument_container.add_argument(
            ...
            default=None,
            metavar="PATH",
            help=("Write v1.0.0 I/O, extraction, initialization, "
...
def run_optimizer(optimizer, *args, **kwargs):
    ...
    Calibration stages call this function at the original optimization callsites.
...
def stage(name, category="optimization", metadata=None):
    ...
    surrounds initialization and I/O phases as well as optimizer calls.
    """
    if _recorder is None:
        yield
        return
...
def finish(status="ok"):
    """Flush and release timing output before its temporary directory is removed."""
    global _recorder
    if _recorder is not None:
