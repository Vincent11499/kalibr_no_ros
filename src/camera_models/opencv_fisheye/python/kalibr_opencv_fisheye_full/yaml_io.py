"""Full OpenCV fisheye YAML conversion, including nonzero alpha/skew.
...
from kalibr_opencv_fisheye import yaml_io as _zero


CAMERA_MODEL = "pinhole_opencv_fisheye"
DISTORTION_MODEL = "opencv_fisheye"
DIRECT_TRANSFORM = _zero.DIRECT_TRANSFORM
INVERSE_TRANSFORM = _zero.INVERSE_TRANSFORM
...
def _optional_real(storage, names):
    node = _zero._node(storage, names)
    if node is None:
        return None
...
def _validate_fisheye_declaration(storage):
    ...
    for field in ("kalibr_distortion_model", "distortion_model"):
        value = _zero._read_string(storage, (field,), "")
        if not value:
            continue
    ...
    camera_model = _zero._read_string(storage, ("camera_model",), "")
    if camera_model:
...
def _read_stereo_transform(storage):
    rotation_node = _zero._node(storage, ("R", "rotation_matrix"))
    translation_node = _zero._node(
    ...
    if rotation_node is not None and translation_node is not None:
        rotation = _reshape_matrix(
            "R",
            _zero._read_matrix(storage, ("R", "rotation_matrix")),
            3,
            3,
        )
        translation = _finite(
    ...
    transform = _reshape_matrix(
        "RT",
        _zero._read_matrix(storage, ("RT", "T_cn_cnm1", "transform")),
        4,
        4,
    )
    if not np.allclose(
...
def _translation_scale_to_meters(
    ...
    stored_unit = _zero._read_string(
...
class OpenCvFisheyeCamera:
    ...
    def __post_init__(self):
        ...
        self.resolution = _zero._validate_resolution(self.resolution)
...
class OpenCvFisheyeStereo:
    ...
    def __post_init__(self):
        self.R = _zero._validate_rotation(self.R)
        self.T = _zero._validate_translation(self.T).reshape(3)
...
def read_opencv_camera(
    ...
    storage = _zero._open_storage(path, cv2.FILE_STORAGE_READ)
    try:
        _validate_fisheye_declaration(storage)
        if prefix:
            K = _zero._read_matrix(
                storage, ("K{}".format(prefix), "M{}".format(prefix))
            )
            D = _zero._read_matrix(storage, ("D{}".format(prefix),))
            camera_name = _zero._read_string(
                storage,
                (
            ...
            camera_topic = topic or _zero._read_string(
                storage,
                (
            ...
            K = _zero._read_matrix(storage, ("K", "camera_matrix"))
            D = _zero._read_matrix(
                storage, ("D", "distortion_coefficients")
            )
            camera_name = name or _zero._read_string(
        ...
        return OpenCvFisheyeCamera(
            K,
            D,
            _zero._read_resolution(storage, prefix, resolution),
            alpha,
            camera_name,
            camera_topic,
        )
...
def read_opencv_stereo(
    ...
    *,
    ...
    storage = _zero._open_storage(stereo_path, cv2.FILE_STORAGE_READ)
    try:
        _validate_fisheye_declaration(storage)
        embedded = _zero._node(storage, ("K1", "M1")) is not None
        rotation, translation = _read_stereo_transform(storage)
        declared_direction = _zero._read_string(
    ...
    if direction == INVERSE_TRANSFORM:
        rotation, translation = _zero.invert_stereo_transform(
...
def _camera_from_entry(entry, index):
    ...
    if model == CAMERA_MODEL:
        ...
        # Backward-compatible import of the zero-skew extension's schema.
        if intrinsics.size != 4:
            raise ValueError("legacy pinhole fisheye intrinsics require 4 values")
        fu, fv, cu, cv = intrinsics
        alpha = 0.0
    else:
        raise ValueError(
            "cam{} is not a full OpenCV fisheye camera (model={!r})".format(
                index, model
            )
        )
...
def read_kalibr_camchain(path):
    ...
    while "cam{}".format(index) in data:
        ...
        if index:
            ...
            transforms.append(
                (
                    _zero._validate_rotation(transform[:3, :3]),
                    _zero._validate_translation(transform[:3, 3]).reshape(3),
                )
            )
        index += 1
...
def _write_values(path, values):
    storage = _zero._open_storage(path, cv2.FILE_STORAGE_WRITE)
    try:
