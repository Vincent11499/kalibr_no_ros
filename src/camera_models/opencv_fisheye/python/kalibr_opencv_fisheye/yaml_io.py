"""Zero-skew OpenCV FileStorage <-> Kalibr camchain conversion helpers.
...
def _resolution_argument(values):
    ...
    if arguments.command == "import-mono":
        outputs = [
            import_opencv_mono(
                arguments.input,
                arguments.output,
                resolution=_resolution_argument(arguments.resolution),
                topic=arguments.topic,
                distortion_model=arguments.distortion_model,
            )
        ]
        ...
        outputs = [
            import_opencv_stereo(
                ...
                resolutions=(
                    _resolution_argument(arguments.left_resolution),
                    _resolution_argument(arguments.right_resolution),
                ),
                topics=(
        ...
        outputs = export_kalibr_camchain(arguments.input, arguments.output_prefix)
