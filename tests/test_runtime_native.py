...
class NativeBoostTypeCompatibilityTest(unittest.TestCase):
    ...
    def test_thread_and_timing_controls_preserve_native_types(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            timing_output = (
                str(timing_path)
                if kalibr_runtime.profiling_enabled() else None
            )
            kalibr_runtime.configure(
                argparse.Namespace(
                    parallelism=None,
            ...
            options.nThreads = 4
            kalibr_runtime.apply_optimizer_threads(options)
            ...
            kalibr_runtime.apply_optimizer_threads(estimator_options)
            self.assertEqual(estimator_options.nThreads, 3)

            kalibr_runtime.finish()
            self.assertEqual(
                timing_path.is_file(),
                kalibr_runtime.profiling_enabled(),
            )
            runtime._reset_for_tests()
