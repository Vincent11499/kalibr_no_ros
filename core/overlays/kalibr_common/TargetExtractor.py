...
def multicoreExtractionWrapper(detector, taskq, resultq, clearImages,
    ...
    while True:
        ...
        try:
            ...
            # Report the failing index instead of silently terminating a
            # worker and leaving the parent blocked on a missing result.
...
def extractCornersFromDataset(dataset, detector, multithreading=False,
    ...
    if multithreading:
        ...
        # Bound decoded images in flight. The upstream implementation fills
        # two Manager queues with the complete dataset before starting any
        # worker, temporarily duplicating every image.
